    ...
    <xsl:template match="*">
        ...
        <xsl:for-each select="./*">
            ...
            <xsl:if test="$upsert='yes' and $table_primary_key!='NONE'">
                <xsl:text> ON CONFLICT (</xsl:text><xsl:value-of select="$table_primary_key"/><xsl:text>) DO UPDATE SET </xsl:text>
                <xsl:for-each select="@*">
                    <xsl:variable name="attr_name" select="lower-case(name())"/>
                    <xsl:variable name="field_name">
                        <xsl:choose>
                    ...
                    <xsl:value-of select="$field_name"/><xsl:text>='</xsl:text><xsl:value-of select="."/><xsl:text>'</xsl:text>
                    <xsl:if test="position()!=last()">,</xsl:if>
                </xsl:for-each>
            </xsl:if>
            <xsl:text>;&#xa;</xsl:text>
